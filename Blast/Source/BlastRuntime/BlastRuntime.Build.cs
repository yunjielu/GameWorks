...
{
	public class BlastRuntime : ModuleRules
	{
		public static string GetBlastLibrarySuffix(ModuleRules Rules)
		{
			if ((Rules.Target.Configuration == UnrealTargetConfiguration.Debug ||
			     Rules.Target.Configuration == UnrealTargetConfiguration.DebugGame) &&
			    Rules.Target.bDebugBuildsActuallyUseDebugCRT)
			{
				return "DEBUG";
			}
			else if (Rules.Target.Configuration == UnrealTargetConfiguration.Development &&
			         Rules.Target.Platform != UnrealTargetPlatform.Linux)
			{
		...
		public static void SetupModuleBlastSupport(ModuleRules Rules, string[] BlastLibs)
		{
			...
			DirectoryReference ModuleRootFolder =
				(new DirectoryReference(Rules.ModuleDirectory)).ParentDirectory.ParentDirectory;
			DirectoryReference EngineDirectory =
				new DirectoryReference(Path.GetFullPath(Rules.Target.RelativeEnginePath));
			string BLASTLibDir = Path.Combine("$(EngineDir)", ModuleRootFolder.MakeRelativeTo(EngineDirectory),
				"Libraries", Rules.Target.Platform.ToString());

			Rules.PublicLibraryPaths.Add(Path.Combine(ModuleRootFolder.ToString(), "Libraries",
				Rules.Target.Platform.ToString()));
		...
		public BlastRuntime(ReadOnlyTargetRules Target) : base(Target)
		{
