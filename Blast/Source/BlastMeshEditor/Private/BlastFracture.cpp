...
TSharedPtr<FFractureSession> FBlastFracture::StartFractureSession(UBlastMesh* InBlastMesh, UStaticMesh* InSourceStaticMesh, UBlastFractureSettings* Settings, bool ForceLoadFracturedMesh)
{
	...
	if (InSourceStaticMesh != nullptr)
	{
		...
		FMeshMergeHelpers::RetrieveMesh(InSourceStaticMesh, 0, DestMeshdescription);
		FMeshDescriptionOperations::ConvertToRawMesh(DestMeshdescription, InSourceRawMesh, ToMaterialMap);
...
{
	FBlastFractureHistory& History = FS->BlastMesh->FractureHistory;
	...
	float Radius, FVector Origin, float Probability, bool ForceReset)
{
	...
	if (VoronoiSitesGenerator.IsValid())
	{
		...
		if (FractureSession->FractureTool->voronoiFracturing(FractureChunkId, sitesCount, sites, NvcVec3({ 1, 1, 1 }), NvcQuat({ 0, 0, 0, 1 }), IsReplace) == 0)
		{
			return true;
		}
	}
